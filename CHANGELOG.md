...
### 新增
- **`/放送时间` 指定周几**: 命令支持可选的 `周X` 参数设置周几(如 `/放送时间 无职 周日 23:00`),同步落库 `broadcast_weekday`。
- **手动锁定防覆盖**: 手动设定时间后该番剧进入锁定状态,`/刷新放送` 会跳过已锁定的记录;查询时显示 🔒 标识;`清空` 命令同步解锁。`broadcast_manual` 列 DB ALTER 自动兼容旧表。

## v1.5.6

### 修复
- **放送进度判定**: 搜索结果卡片渲染剧集进度时,有合法 `airdate` 的剧集改为只按播出日期判断是否已播,避免 Bangumi 未来集已有评论数时被误标为已放送。
- **订阅更新通知发送**: 订阅更新推送改用 AstrBot `Context.send_message` 主动发送接口,避免 AstrBot 4.26.2 中不存在 `StarTools.send_message_by_id` 时更新通知发送失败。

### 新增
- **番剧简介自动翻译**: 新增 `auto_translate_subject_summary` 配置,`/bgm` 搜索结果卡片渲染前会识别简介语言;纯中文简介保持原文,含日文假名的日文或中日混合简介会调用 AstrBot 默认聊天模型翻译为中文。

### 更新
- **版本同步**: 将插件元数据、README 版本徽章、预览脚本报告版本和本地预览输出目录推进到 `v1.5.6`。
...
### 修复
- **README 版本徽章**: 将 README 顶部 SVG 版本徽章同步到 `v1.5.3`,避免发布页继续展示旧版本。
- **README 示例图片地址**: 将渲染示例图片从仓库内相对路径改为 GitHub raw 网络地址,避免外部渲染环境无法解析相对文件路径时图片不显示。

### 更新
- **版本同步**: 将插件元数据、预览脚本报告版本和 README 本地预览输出目录推进到 `v1.5.3`。

## v1.5.2

### 新增
- **README 渲染示例**: 增加搜索结果卡片、单集更新卡片、长文本响应卡片和每日放送表 demo 图,未下载插件也能直接预览渲染效果。

### 更新
- **品牌图标**: 更新插件 logo,使用更鲜明的 `BANGUMI` 标题视觉。


## v1.5.1

### 修复
- **代理渲染链路**: 配置 `proxy_http` 和 `port` 后,代理现在会同时覆盖 Bangumi API、远程 RPC 渲染、本地 Playwright 渲染和 Pillow 封面图片下载。国内服务器或无法直连 `lain.bgm.tv` 的环境中,搜索成功后生成卡片不再因为渲染阶段绕过代理而超时或失败。

### 优化
- **代理回归测试**: 新增代理传递测试,覆盖插件初始化、搜索结果卡、单集更新卡、放送表、RPC 请求、Playwright 启动和 Pillow 图片下载,避免后续改动再次漏掉渲染链代理。
