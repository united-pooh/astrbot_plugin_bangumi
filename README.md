...
| `/放送时间` | 查看所有已订阅番剧的放送时间（按周分组,支持30h制）;或查询/设置/清除指定番剧 | `[关键词\ID] [周X] [HH:MM\|清空]` | `/放送时间` 查看全部,`/放送时间 尖帽子 24:28` 设置,`/放送时间 尖帽子 周五 24:28` 带周几,`/放送时间 尖帽子 清空` 清除 |
| `/刷新放送` | 从 bgmlist 手动刷新所有已订阅番剧的放送时间(已手动锁定的番剧会被跳过) | 无 | `/刷新放送` |
| `/放送测试` | (调试) 模拟一次推送;无参仅测 send_message,带 ID 或番剧名跑完整 check_updates | `[ID\|番剧名]` | `/放送测试` 或 `/放送测试 描绘直至` |
...
| `broadcast_time_30h` | bool | `true` | 30小时制显示深夜档(如 周五00:28 → 周四24:28);关闭后回落24h制 |
